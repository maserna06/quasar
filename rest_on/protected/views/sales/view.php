...
$this->menu = array(
    ...
    array('label' => 'Actualizar Venta', 'url' => array('update', 'id' => $model->sale_id)),
...
Yii::app()->clientScript->registerScript("export", "
        $('#export_pdf').click(function(){
            if(confirm('" . Yii::t('application', 'Generation time can take up to 5 minutes, depending on the number of records. Want to continue?') . "')){
                window.open('" . Yii::app()->controller->createUrl('exportPdf', array('type' => 'PDF', 'id' => $model->sale_id)) . "', '', 'hotkeys=no, height=500, width=700');
            }
            return false;
        });
