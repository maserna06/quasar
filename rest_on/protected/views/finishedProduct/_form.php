    ...
    <div class="col-xs-12 col-md-4 col-lg-4 datos-generales" style="display: none;" id="datos-generales">
        <div class="panel panel-default height">
            ...
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-12">
                        <label><?php echo $form->labelEx($model, 'finished_product_date'); ?></label>
                        <div class="input-group">
                            <div class="input-group-addon">
                            ...
                            $this->widget('zii.widgets.jui.CJuiDatePicker', array(
                                'model' => $model,
                                'attribute' => 'finished_product_date',
                                'language' => 'es',
                                'options' => array(
                                    'dateFormat' => 'yy-mm-dd',
                                ),
                                'htmlOptions' => array(
                                    ...
                                    'value' => ($model->finished_product_date) ? $model->finished_product_date : date('Y-m-d'),
                                ),
                            ));
                            ?>
                        </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="input-group">
                            ...
                            ?>
                            <?php echo $form->hiddenField($model, 'finished_product_status', array('class' => 'form-control', 'value' => '1')); ?>
                        </div>
                    </div>                            
                </div>
                <br>
                <div class="row">
                    <div class="col-md-12">
                        <label><?php echo $form->labelEx($model, 'finished_product_remarks'); ?></label>                                
                        <div class="input-group">
                            <div class="input-group-addon">
                                <i class="fa fa-sticky-note-o"></i>
                            </div>
                            <?php echo $form->textArea($model, 'finished_product_remarks', array('size' => 60, 'maxlength' => 500, 'class' => 'form-control')); ?>
                        </div>
                    </div>
                </div>
                <!--<br>
...
<script>
    ...
    function detalleProducts(myArr) {
        ...
        for (i = 0; i < myArr.length; i++) {
            ...
            container.appendChild(tr);
            var iden = 'trprod' + next_inc;
            $('#' + iden).load("<?= Yii::app()->baseUrl ?>/finishedProduct/detailView?id_detail=" + myArr[i] + '&cantidad=' + next_inc, function (response, status, xhr) {
                if (response == '') {
                    container.removeChild(tr);
                    next_inc = next_inc - 1;
                    real = real - 1;
                ...
                if (accion == 'view') {
                    $('#finished-product-form').find('input, textarea, button, select').attr('disabled', 'disabled');
                    $('#finished-product-form').find('a').removeAttr("onclick");
                }
                bruto();
            });
        }
...
//            validateComponents(true);
        ...
        function viewProducts(e) {
            ...
            $("#bodyArchivos").html('');
            $.ajax({
                url: "<?php echo $this->createUrl('/finishedProduct/ViewProducts') ?>",
                success: function (result) {
                    $("#bodyArchivos").html(result);
                    $("#myModal").modal({keyboard: false});
        ...
        (function ($) {
            ...
            var projects = function (request, response) {
                $.ajax({
                    url: "<?php echo $this->createUrl('/products/autofinish') ?>",
                    dataType: "json",
                    data: {
                        term: request.term,
                    },
    ...
    function addProd(idprod) {
        ...
        container.appendChild(tr);
        var iden = 'trprod' + next_inc;
        $('#' + iden).load("<?= Yii::app()->baseUrl ?>/products/productfinish?id_prod=" + idprod + '&cantidad=' + next_inc, function (response, status, xhr) {
            if (response == '') {
                container.removeChild(tr);
                next_inc = next_inc - 1;
                real = real - 1;
    ...
    function guardar() {
        ...
        var date = $('#FinishedProduct_finished_product_date').val();
        if (!date) {
            $('#FinishedProduct_finished_product_date').addClass("error-comp");
            error++;
        } else {
            $('#FinishedProduct_finished_product_date').removeClass("error-comp");
        }
        if (error > 0 || real < 1) {
            var dato = new Array();
