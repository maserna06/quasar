 ...
 */
class TransferConfig extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('transfer_id, transfer_format, wharehouse_in, wharehouse_out', 'required','on'=>'insert, update,existe'),
			array('transfer_id, transfer_format, wharehouse_in, wharehouse_out', 'numerical', 'integerOnly'=>true),
			array('transfer_id', 'validate_num', 'on' => 'existe'),
	...
	public function validate_num($attr, $param) {
        $valor = TransferConfigExtend::transfersCreate();
        if($valor)
            $valor = TransferConfig::model()->findByPk($valor)->transfer_id;
        else
            $valor = 0;
        if ($valor > $this[$attr]) {
	...
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'transfer_id' => 'Consecutivo',
			'transfer_format' => 'Formato',
			'wharehouse_in' => 'Bodega Entrada',
			'wharehouse_out' => 'Bodega Salida',
		);
	}
