 ...
 */
class FinishedProductConfig extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('finished_product_id, finished_product_format, wharehouse_id', 'required','on'=>'insert, update,existe'),
			array('finished_product_id, finished_product_format, wharehouse_id', 'numerical', 'integerOnly'=>true),
			array('finished_product_id', 'validate_num', 'on' => 'existe'),
	...
	public function validate_num($attr, $param) {
        $valor = FinishedProductConfigExtend::finishedproductCreate();
        if($valor)
            $valor = FinishedProductConfig::model()->findByPk($valor)->finished_product_id;
        else
            $valor = 0;
        if ($valor > $this[$attr]) {
	 ...
	 * @return FinishedProductConfig the static model class
	 */
